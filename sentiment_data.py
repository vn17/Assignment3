...
def read_blind_sst_examples(infile: str, lower_case:bool = True) -> List[List[str]]:
    """
    Reads the blind SST test set, which just consists of unlabeled sentences
    :param infile: path to the file to read
    :param lower_case: whether to lowercase the data
    ...
    for line in f:
        if len(line.strip()) > 0:
            ...
            if lower_case:
                sent = sent.lower()
            # Tokenize the sentence and remove empty strings
            tokenized_cleaned_sent = list(filter(lambda x: x != '', sent.split(" ")))
